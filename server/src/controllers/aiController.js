  ...
  try {
    ...
    if (shortDescription.trim().length < 30) {
      return res.status(400).json({
        message: "Please provide a more detailed description (at least 30 characters) for accurate AI evaluation.",
      });
    }
...
`;
    ...
    try {
      ...
      evaluation = JSON.parse(retryText.replace(/```json|```/g, "").trim());
    }
