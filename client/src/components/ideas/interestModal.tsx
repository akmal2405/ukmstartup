...
export default function InterestModal({ isOpen, onClose, ideaId, startupName }: { isOpen: boolean; onClose: () => void; ideaId: number; startupName: string; }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/interests/${ideaId}`, {
        method: "POST",
        headers: {
